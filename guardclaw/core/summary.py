...
def build_summary_from_engine(
    ...
    replay_summary = (
        verification_summary if verification_summary is not None
        else engine.verify()
    )
    ...
    entries: List[Dict[str, Any]] = [
        {
            ...
            "record_type": (
                env.record_type.value
                if hasattr(env.record_type, "value")
                else env.record_type
            ),
    ...
    agent_ids = sorted({
        env.agent_id for env in engine.envelopes
        if getattr(env, "agent_id", None)
    })
    ...
    return {
        ...
        "failure_type": (
            str(getattr(replay_summary, "failure_type", None))
            if getattr(replay_summary, "failure_type", None) else None
        ),
...
def build_summary(ledger_path: str | Path) -> Dict[str, Any]:
    ...
    # Fix: removed stale parallel= argument
