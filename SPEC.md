...
This document specifies **GEF Protocol Version 1.0**, which is the first stable release of the specification. It is published alongside the reference implementation (`guardclaw` v0.6.1, Python) and a verified cross-language proof demonstrating byte-identical behavior between the Python reference implementation and an independent Go implementation.
...
- [1. Introduction](#1-introduction)
  - [1.1 Why a Protocol, Not a Library](#11-why-a-protocol-not-a-library)
- [2. Design Goals](#2-design-goals)
- [3. Terminology](#3-terminology)
- [4. Protocol Overview](#4-protocol-overview)
- [5. The Envelope](#5-the-envelope)
  - [5.1 Field Definitions](#51-field-definitions)
...
- [7. Signing and Verification Procedure](#7-signing-and-verification-procedure)
  ...
  - [7.3 Verifying an Envelope Signature](#73-verifying-an-envelope-signature)
  - [7.4 Verifying a Full Ledger](#74-verifying-a-full-ledger)
- [8. Chain Construction](#8-chain-construction)
  - [8.1 Genesis Entry](#81-genesis-entry)
  - [8.2 Subsequent Entries](#82-subsequent-entries)
  - [8.3 Chain Hash Algorithm](#83-chain-hash-algorithm)
- [9. The Ledger Format](#9-the-ledger-format)
  - [9.1 Encoding](#91-encoding)
...
- [11. Security Considerations](#11-security-considerations)
  ...
  - [11.3 Attack Surface Analysis](#113-attack-surface-analysis)
  - [11.4 Known Limitations](#114-known-limitations)
  - [11.5 Ledger Anchoring](#115-ledger-anchoring)
- [12. Cross-Language Compliance](#12-cross-language-compliance)
  - [12.1 Compliance Requirements](#121-compliance-requirements)
  - [12.2 Test Vectors](#122-test-vectors)
  - [12.3 Verified Implementations](#123-verified-implementations)
...
- [16. Non-Normative Design Rationale](#16-non-normative-design-rationale)
  - [16.1 Why a Chain Hash, Not a Merkle Tree](#161-why-a-chain-hash-not-a-merkle-tree)
  - [16.2 Why the Chain Hash Excludes the Signature](#162-why-the-chain-hash-excludes-the-signature)
  - [16.3 Why Ed25519 and Not RSA or ECDSA (P-256)](#163-why-ed25519-and-not-rsa-or-ecdsa-p-256)
  - [16.4 Why RFC 8785 JCS and Not Protocol Buffers or MessagePack](#164-why-rfc-8785-jcs-and-not-protocol-buffers-or-messagepack)
  - [16.5 Why JSONL and Not a Database Format](#165-why-jsonl-and-not-a-database-format)
  - [16.6 Why Ten Signing Fields and Not More](#166-why-ten-signing-fields-and-not-more)
  - [16.7 Why Apache 2.0 and Not MIT or GPL](#167-why-apache-20-and-not-mit-or-gpl)
- [17. References](#17-references)
- [Appendix A: Field Quick Reference](#appendix-a-field-quick-reference)
- [Appendix B: Violation Type Registry](#appendix-b-violation-type-registry)
- [Appendix C: Record Type Registry and Governance](#appendix-c-record-type-registry-and-governance)
- [Appendix D: Signing Surface Field Order (Informative)](#appendix-d-signing-surface-field-order-informative)
...
| `sequence` | integer | Zero-based monotonically increasing position in this agent's ledger. MUST be a non-negative integer. |
...
The SHA-256 digest is encoded as exactly 64 lowercase hexadecimal characters. Uppercase hex is NOT permitted in `causal_hash` values.
...
| ID | Invariant |
|---|---|
| **INV-23** | The replay engine MUST detect a chain break in a JSONL ledger and report it as a `chain_break` violation with the correct sequence position. |
...
| Threat | Description | Detection Mechanism |
|---|---|---|
| **T-1** | Post-hoc record modification | Signature verification (Contract III) and chain verification (Contract II) |
| **T-2** | Record insertion | Chain verification: the inserted entry's `causal_hash` will not match its predecessor |
| **T-3** | Record deletion | Sequence gap detection (INV-25) and chain verification |
| **T-4** | Record reordering | Chain verification and sequence gap detection |
| **T-5** | Signature forgery | Computationally infeasible for Ed25519 under current cryptographic assumptions |
| **T-6** | Key substitution | Changing `signer_public_key` breaks the chain hash of the next entry |
| **T-7** | Protocol downgrade | `gef_version` is in the signing surface; mutating it breaks signature verification |
| **T-8** | Cross-ledger replay | Chain verification (`causal_hash` mismatch) and nonce uniqueness |
...
> **Implementation note.** Anchoring is orthogonal to the GEF protocol. It does not change the envelope format, the signing procedure, or the verification procedure. It is an operational practice layered on top of a valid GEF ledger.
...
10. Produce byte-identical canonical bytes and chain hashes for all test vectors in Section 12.2.
...
|---|---|---|---|
| Python (reference) | https://github.com/viruswami5511/guardclaw | v0.6.1 | Stable |
| Go | https://github.com/viruswami5511/guardclaw/tree/master/cross_lang_proof | v0.6.1 | Verified (proof bundle) |
...
| Language | Recommended call |
|---|---|
| Python | `secrets.token_hex(16)` |
| Node.js | `crypto.randomBytes(16).toString('hex')` |
| Go | `crypto/rand` package |

Never use `random.random()` or a seeded PRNG.

**Use millisecond precision for timestamps.**
`datetime.utcnow()` in Python returns microseconds. Truncate to milliseconds before formatting:
...
An unsigned envelope (no `signature` field) is not a valid GEF entry. The pattern `env = Envelope.create(...).sign(key)` should be enforced by the API.
...
> (GEF-SPEC-1.0), published 2026-02-27.
>
...
> This section is **non-normative**: nothing in this section creates requirements. Implementors and evaluators may find it useful for understanding why the protocol is specified as it is.
...
| Reference | Citation |
|---|---|
| **[RFC 2119]** | Bradner, S., "Key words for use in RFCs to Indicate Requirement Levels", RFC 2119, March 1997. https://www.rfc-editor.org/rfc/rfc2119 |
...
| **[RFC 8032]** | Josefsson, S. and I. Liusvaara, "Edwards-Curve Digital Signature Algorithm (EdDSA)", RFC 8032, January 2017. https://www.rfc-editor.org/rfc/rfc8032 |
...
| **[FIPS-180-4]** | National Institute of Standards and Technology, "Secure Hash Standard (SHS)", FIPS PUB 180-4, August 2015. https://doi.org/10.6028/NIST.FIPS.180-4 |
| **[JSONL]** | JSON Lines format specification. https://jsonlines.org |
...
The authoritative record type registry is this appendix. A `record_type` value is valid if and only if it appears in the registered record types table of the `gef_version` claimed by the envelope.
